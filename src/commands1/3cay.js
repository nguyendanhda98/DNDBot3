...
export async function execute(message, args, cmd, client, discord, profileData) {
    ...
    switch (args[0]) {
                    ...
                    addFields: [
                        ...
                        },
                        {
                            ...
                            value: result.players
                                .map(
                                    (player) => `${player.username} (${player.betAmount})`
                                )
                                .join('\n'),
                        },
                    ],
        ...
        case 'join': // join <@Chủ phòng>
            ...
            if (checkJoined) {
                extra = {
                    setDescription: checkJoined.host.id === host.id
        ...
        case 'bet': // bet <So tien>
            ...
            extra = {
                setDescription: +args[1] === profileData.cash
        ...
        case 'start':
            ...
            const getStartEditEmbed = (numberCard, showAmount = false) => {
                extra.addFields = result.playersInfo.map((playerCard) => ({
                    name: `${playerCard.username}${showAmount ? `\n${playerCard.showAmount}` : ''}`,
